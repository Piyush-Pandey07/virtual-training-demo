 ...
 *
 * Anybody on the company domain can now enrol themselves, which is the point — but
 * they arrive as a trainee and there is no path from there to administrator that does
 * not go through somebody who already is one. Two ways in, both deliberate:
 *
 *   - an address named in AUTH_ADMIN_EMAILS, which only whoever deploys can change
 *   - "Make HR" on the people screen, which only an existing administrator can reach
 *
...
describe('the sign-in routes', () => {
  ...
  it('grant admin only from the deployment list', () => {
    // register promotes exactly once, guarded by isBootstrapAdmin. If setRole ever
    // appears there under a different condition, this is the test that should fail.
    ...
    assert.match(source, /if \(!known && isBootstrapAdmin\(email\)\) await store\.setRole/);
  });
