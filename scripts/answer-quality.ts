...
const COMPREHENSION_CHECK =
  /((does|did|do|would|is|was) (that|it|this|these|you)\b[^?]{0,60}(help|make sense|clear|clarify|answer|useful|follow|understand)|make sense|makes sense|hope that helps|any(thing else|thing further| other questions| questions)|clear\?|understood\?|sound(s)? (good|ok|right)\?)/i;
...
async function main() {
  for (const [kind, question] of SCRIPT) {
    ...
    rows.push({
      ...
      closing,
      generic: COMPREHENSION_CHECK.test(closing),
