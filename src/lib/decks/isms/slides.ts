/**
 * The ISO 27001 ISMS Awareness deck.
 *
 ...
 * in `internalNotes`, which reaches neither the model nor the browser. Slides 2
 * and 4 are the reason that distinction exists.
 *
 * This is data now, not the definition of a deck. The shape it satisfies lives in
 * src/lib/deck-types.ts, and the engine reads whichever deck it is handed.
 */

import type { DeckSlide } from '../../deck-types';

export const ISMS_SLIDES: DeckSlide[] = [
  {
    id: 1,
  ...
  },
  {
    ...
    narrationBrief:
      'This is the most practically useful slide in the deck, so give it room. Take the four tiers in order and pair each one with its impact test from the presenter notes, because the impact test is what people actually use to decide. Then cover the three obligations: handling, labelling, storage. Give the red warning line real weight, because it is the single rule most often broken: personal accounts must never be used for corporate or client information. Be clear that it is absolute, and be kind about it, since almost everyone who breaks it is trying to get work done rather than being careless. Mention the OneDrive recommendation with sync turned on. If the trainee asks for an example, draw one from data centre consultancy work.',
    keyPoints: [
